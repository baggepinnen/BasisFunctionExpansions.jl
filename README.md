...
```
![window](figs/arx.png)
Du to the non-linearity at the input of the system, the linear model fails to fit the data well. Our next attempt is a non-linear model based on BFEs. We select the simplest form of multi-dimensional BFE, `MultiUniformRBFE` and further select to cover the state-space with 2 basis functions along each dimension dimension corresponding to `y`, and 4 basis functions along each dimension dimension corresponding to `u` for a total of 2^2*4^2=64 parameters (4 basis functions is the smallest number that can accurately fit the `√(|u|)`). The number of parameters in this case is large compared to the number of data points, we may need some regularization to fit this model properly. The regularization choice is made when forming the `BasisFunctionApproximation` and the strength is determined by the last argument `1e-2` in this case.
```julia
